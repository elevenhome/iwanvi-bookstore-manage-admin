    ...
    $.extend({
    	...
    	table: {
            ...
            init: function(options) {
                ...
                $('#' + options.id).bootstrapTable({
                    ...
                    pageList: [10, 30, 50, 100, 300, 500, 1000, 3000, 5000],        // 可供选择的每页的行数（*）
    		...
    		// 导出所有数据
    		exportAllExcel: function(formId) {
				$.modal.confirm("确定导出所有" + $.table._option.modalName + "吗？", function() {
					var currentId = $.common.isEmpty(formId) ? $('form').attr('id') : formId;
					$.modal.loading("正在导出数据，请稍后...");
					$.post($.table._option.exportAllUrl, $("#" + currentId).serializeArray(), function(result) {
						if (result.code == web_status.SUCCESS) {
							window.location.href = ctx + "common/download?fileName=" + result.msg + "&delete=" + true;
						} else {
							$.modal.alertError(result.msg);
        ...
        operate: {
			...
			sync: function(id) {
				...
				// 	return;
				// }
				$.modal.confirm("确认要同步选中的数据吗?", function() {
					var url = $.table._option.syncUrl;
					var data = { "id": id };
					$.operate.submit(url, "post", "json", data);
				});
			}
